...
const SentryHeader: React.FC<SentryHeaderProps> = ({
  ...
  return <header className="sentry-panel flex items-center justify-between h-auto min-h-[100px] mb-4 py-3 px-6">
      {/* Left side - Logo */}
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        <Link to="/workspace" className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-sentry-primary/30 hover:bg-sentry-primary/10 transition-colors" title="Workspace">
          <Layout className="h-4 w-4 text-sentry-primary" />
          <span className="text-xs text-sentry-accent hidden sm:inline">Workspace</span>
        </Link>

        <Link to="/settings" className="hover:text-sentry-primary transition-colors p-1.5" title="Settings">
