...
const SentryHeader: React.FC<SentryHeaderProps> = ({
  ...
  return <header className="sentry-panel flex items-center justify-between h-auto min-h-[120px] mb-4 py-3 px-6">
      {/* Left side - Logo */}
      <div className="flex-shrink-0 flex items-center">
        <img alt="YOD ALEF Engineering Logo" src={logo} style={{
        width: '100px',
        height: '114px',
        filter: 'brightness(1.3) contrast(1.1) saturate(1.1) drop-shadow(0 0 12px rgba(0, 255, 136, 0.3))'
      }} className="shadow-inner border border-double border-yellow-400 rounded-3xl opacity-95 object-contain" />
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4">
        ...
        <div className="flex items-center gap-2 mt-3">
          ...
          return <Link key={link.to} to={link.to} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded border transition-colors text-xs", isActive ? "border-sentry-primary bg-sentry-primary/20 text-sentry-primary" : "border-sentry-primary/30 hover:bg-sentry-primary/10 text-sentry-accent")}>
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex flex-col items-end gap-1">
          <div className="flex items-center gap-2">
            ...
            <div className={cn("h-2.5 w-2.5 rounded-full animate-pulse", systemStatus === 'active' && "bg-green-500", systemStatus === 'standby' && "bg-yellow-500", systemStatus === 'warning' && "bg-orange-500", systemStatus === 'error' && "bg-red-500")} />
          </div>
          <span className={cn("text-xs font-mono uppercase", systemStatus === 'active' && "text-green-500", systemStatus === 'standby' && "text-yellow-500", systemStatus === 'warning' && "text-orange-500", systemStatus === 'error' && "text-red-500")}>
            {systemStatus}
          </span>
        </div>
